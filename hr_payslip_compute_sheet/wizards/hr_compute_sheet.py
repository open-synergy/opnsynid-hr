...
class HrComputeSheet(models.TransientModel):
    ...
    reload_input = fields.Boolean(string="Reload Input", default=False)

    reload_workdays = fields.Boolean(string="Reload Workdays", default=False)
    ...
    def _reload_input(self, payslip):
        ...
        if input_line_ids:
            for input_line in payslip.input_line_ids:
                reset = {"input_line_ids": [(2, input_line.id)]}
                payslip.write(reset)
            vals = {"input_line_ids": [(0, 0, input_line_ids[0])]}
            payslip.write(vals)
    ...
    def _reload_workdays(self, payslip):
        ...
        if worked_days_line_ids:
            for worked_days in payslip.worked_days_line_ids:
                reset = {"worked_days_line_ids": [(2, worked_days.id)]}
                payslip.write(reset)
            vals = {"worked_days_line_ids": [(0, 0, worked_days_line_ids[0])]}
            payslip.write(vals)
    ...
    def button_compute_sheet(self):
        ...
        for payslip in obj_hr_payslip.browse(record_ids):
            if payslip.state == "draft":
                ...
                raise UserError(
                    _("Payslip No. %s " "cannot be computed " "because state is done.")
                    % (payslip.number,)
                )
