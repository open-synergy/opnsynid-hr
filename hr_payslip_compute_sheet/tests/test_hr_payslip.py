...
class TestHRPayslip(TransactionCase):
    def setUp(self, *args, **kwargs):
        ...
        # Data
        self.country = self.env.ref("base.id")
        ...
        self.rule_1 = self.env.ref("hr_payroll.hr_salary_rule_houserentallowance1")
        self.rule_2 = self.env.ref("hr_payroll.hr_salary_rule_convanceallowance1")
        self.rule_3 = self.env.ref("hr_payroll.hr_salary_rule_professionaltax1")
        self.rule_4 = self.env.ref("hr_payroll.hr_salary_rule_providentfund1")
        self.rule_5 = self.env.ref("hr_payroll.hr_salary_rule_meal_voucher")
        self.rule_6 = self.env.ref("hr_payroll.hr_salary_rule_sales_commission")
    ...
    def test_compute_sheet(self):
        ...
        new.write(
            {"worked_days_line_ids": lst_worked_days, "input_line_ids": lst_inputs}
        )
        ...
        self.assertNotEqual(check_nod, new.worked_days_line_ids.number_of_days)

        # Modify Amount
        new.input_line_ids[0].amount = 100
        # Check Amount
        self.assertNotEqual(check_amount, new.input_line_ids[0].amount)

        # Create Compute Sheet
        wiz = self.wiz.with_context(active_ids=[new.id])

        wiz.create(
            {"reload_workdays": True, "reload_input": True}
        ).button_compute_sheet()

        # Check Reload Worked Days
        self.assertEqual(check_nod, new.worked_days_line_ids.number_of_days)
        # Check Reload Inputs
        self.assertEqual(check_amount, new.input_line_ids[0].amount)

        # Change State = Done
        new.signal_workflow("hr_verify_sheet")

        self.assertEqual("done", new.state)

        # Create Compute Sheet
        wiz = self.wiz.with_context(active_ids=[new.id])

        msg = ("Payslip No. %s " "cannot be computed " "because state is done.") % (
            new.number,
        )
