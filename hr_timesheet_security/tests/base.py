...
class BaseCase(TransactionCase):
    def setUp(self, *args, **kwargs):
        ...
        self.date_to = date_now + timedelta(days=7)
        ...
        self.dept_SE = self.obj_department.create(
            {"name": "Software Engineer", "parent_id": self.dept_IT.id}
        )
        ...
        self.dept_RCR = self.obj_department.create(
            {"name": "Recruitment", "parent_id": self.dept_HR.id}
        )
        # Departement Compensation and Evaluations
        self.dept_CAE = self.obj_department.create(
            {"name": "Compensation and Evaluations", "parent_id": self.dept_HR.id}
        )
        # Departement Training and Development
        self.dept_TD = self.obj_department.create(
            {"name": "Training and Development", "parent_id": self.dept_HR.id}
        )
        ...
        self.dept_PS = self.obj_department.create(
            {"name": "Payroll Services", "parent_id": self.dept_FA.id}
        )
        # Departement Cash Office
        self.dept_CO = self.obj_department.create(
            {"name": "Cash Office", "parent_id": self.dept_FA.id}
        )

        # Employee 1 With Timesheet Security User Groups
                ...
                "groups_id": [
                    (
                        ...
                        [
                            ...
                            self.env.ref(
                                "hr_timesheet_security.group_timesheet_operation_user"
                            ).id,
                        ],
                    )
                ],
                    ...
                    (
                        ...
                        [
                            ...
                            self.env.ref(
                                ...
                                "group_timesheet_operation_supervisor"
                            ).id,
                            self.env.ref(
                                "hr_timesheet_security.group_timesheet_viewer_supervisor"
                            ).id,
                        ],
                    )
                ],
                ...
                "groups_id": [
                    (
                        ...
                        [
                            ...
                            self.env.ref(
                                "hr_timesheet_security."
                                "group_timesheet_operation_department"
                            ).id,
                            self.env.ref(
                                "hr_timesheet_security.group_timesheet_viewer_department"
                            ).id,
                        ],
                    )
                ],
                ...
                "groups_id": [
                    (
                        ...
                        [
                            self.grp_employee.id,
                            self.env.ref(
                                "hr_timesheet_security.group_timesheet_operation_company"
                            ).id,
                            self.env.ref(
                                "hr_timesheet_security.group_timesheet_viewer_company"
                            ).id,
                        ],
                    )
                ],
        ...
        self.user_10 = self.obj_user.sudo().create(
            {
                ...
                "company_ids": [(6, 0, [self.opnsynid_company.id])],
        ...
        self.user_11 = self.obj_user.sudo().create(
            {
                ...
                "company_ids": [(6, 0, [self.koi_company.id])],
