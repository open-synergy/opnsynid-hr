...
class HrPayslip(TransactionCase):
    def setUp(self, *args, **kwargs):
        ...
        self.obj_type = self.env["hr.payslip.input_type"]
        self.obj_contract = self.env["hr.contract"]
        self.obj_contract_input_type = self.env["hr.contract.input_type"]
    ...
    def test_payslip_compute(self):
        ...
        onchange = payslip.onchange_employee_id(
            payslip.date_from, payslip.date_to, self.employee.id, contract.id
        )
        input_data = []
        ...
        self.assertEqual(len(payslip.input_line_ids), 2)
        for payslip_input in payslip.input_line_ids:
            if payslip_input.code == "X1":
                self.assertEqual(payslip_input.amount, 77.0)
            elif payslip_input.code == "X2":
                self.assertEqual(payslip_input.amount, 88.0)
