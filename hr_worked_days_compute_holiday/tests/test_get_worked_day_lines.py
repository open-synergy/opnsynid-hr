...
class TestGetWorkedDayLines(TransactionCase):
    ...
    def _create_holiday(self):
        holiday = self.obj_holiday.create({"year": 2017, "country_id": self.country.id})
        self.obj_holiday_line.create(
            {"name": "Test Holiday #1", "date": "2017-02-10", "year_id": holiday.id}
        )

        return True

    def _create_employee(self):
        vals_home_addr = {"name": "Home Address Alan", "country_id": self.country.id}
        addr = self.obj_res_partner.create(vals_home_addr)
        vals = {
            "name": "Alan",
    ...
    def test_get_worked_day_lines(self):
        ...
        # Create Contract
        contract = self._create_contract(employee=employee, struct=struct)
        ...
        worked_days_line_ids = onchange["value"]["worked_days_line_ids"][0]
